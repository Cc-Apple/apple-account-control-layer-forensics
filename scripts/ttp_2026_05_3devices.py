...
Purpose:
  Run one structured TTP-alignment, structure-alignment, and
  benign-explanation stress analysis pass against the 2026-05 logs
  for 12G, 15G, and iPhone11Pro.
...
Output:
  C:\Users\Administrator\Desktop\05\Result\TTP_2026_05_3DEV_<timestamp>

Safety:
  Read-only against input folders.
  No deletion.
  No modification.
  No renaming.
  No moving.
...
Note:
  These paths reflect the original local execution environment and may be
  adjusted by reviewers before rerunning the script.
"""
    ...
    for idx, p in enumerate(all_files, 1):
        ...
        try:
            ...
            if is_debug_excluded(rel):
                excluded.append({"device": device, "path": str(p), "reason": "excluded_debug_sysdiagnose_gz_stacks_tailspin"})
                continue

            if is_strict_excluded(rel):
                excluded.append({"device": device, "path": str(p), "reason": "excluded_report_summary_ocr_note_derived"})
                continue

            if not is_candidate(name):
                excluded.append({"device": device, "path": str(p), "reason": "excluded_unsupported_extension"})
                continue
            ...
            if name.lower().endswith(".zip"):
                ...
                try:
                    with zipfile.ZipFile(p) as z:
                        for member in z.namelist():
                            ...
                            if is_debug_excluded(display):
                                excluded.append({"device": device, "path": display, "reason": "excluded_zip_debug_sysdiagnose_gz"})
                                continue

                            if is_strict_excluded(display):
                                excluded.append({"device": device, "path": display, "reason": "excluded_zip_derived_material"})
                                continue

                            if not is_candidate(member):
                                excluded.append({"device": device, "path": display, "reason": "excluded_zip_unsupported_entry"})
                                continue

                            source_type, weight = classify_source_type(display)

                            if source_type == "text_export_non_primary":
                                excluded.append({"device": device, "path": display, "reason": "excluded_zip_text_export_non_primary"})
                                continue

                            if weight <= 0:
                                excluded.append({"device": device, "path": display, "reason": source_type})
                                continue
                    ...
                    errors.append({"device": device, "path": str(p), "member": "", "error": str(e)})
            ...
            if source_type == "text_export_non_primary":
                excluded.append({"device": device, "path": str(p), "reason": "excluded_text_export_non_primary"})
                continue

            if weight <= 0:
                excluded.append({"device": device, "path": str(p), "reason": source_type})
                continue

            text = read_text_file(p)

            if not text:
                errors.append({"device": device, "path": str(p), "member": "", "error": "read_empty_or_failed"})
                continue
...
def aggregate_daily(records):
    ...
    for (device, day), rs in sorted(groups.items()):
        ...
        if day in ["2026-05-05", "2026-05-10", "2026-05-20", "2026-05-21"]:
            anchor = "2026_05_core_anchor_candidate"

        rows.append({
            "device": device,
            "date": day,
            "file_count": len(rs),
            "source_type_summary": ";".join(f"{k}:{v}" for k, v in stypes.most_common()),
            ...
            "apt32_legacy_primary": sumf("apt32_legacy_score_primary"),
...
def select_daily(rows):
    ...
    for r in rows:
        ...
        layers = float(r.get("layer_count_daily", 0) or 0)
        ...
        priority = (
            ...
            + float(r.get("apt32_legacy_primary", 0) or 0) * 0.25
            + layers * 500
        )
        ...
        if layers >= 7:
            reasons.append("7_layers_or_more")
        elif layers >= 6:
            reasons.append("6_layers_or_more")
        if mdm_false > 0 and restriction > 0:
...
def select_files(records, daily_selected):
    selected_day_keys = set((r.get("device"), r.get("date")) for r in daily_selected)
    out = []
    siri = []
    rtc = []
    text = []
...
FILE_HEADERS = [
    "v4_priority", "v4_reason",
    "device", "date", "log_family", "source_type",
    "relative_path", "display_name", "size", "sha256", "zip_sha256",
...
DAILY_HEADERS = [
    "v4_priority", "v4_reason",
    "device", "date", "file_count",
    "source_type_summary", "top_families",
    ...
    "apt42_style_primary", "apt32_legacy_primary",
    "control_layer_primary", "seam_failure_primary",
    "normal_explanation_stress", "layer_count_daily", "anchor", "layer_flags"
]

def write_summary(path, records, excluded, errors, daily, daily_selected, files_selected, siri, rtc, text):
    dev_counts = Counter(r.get("device") for r in daily_selected)
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write("  This run analyzes only 2026-05 logs for 12G, 15G, and iPhone11Pro.\n")
        f.write("  This is not an attribution assessment.\n\n")
        f.write("Counts:\n")
        f.write(f"  Parsed files: {len(records)}\n")
        ...
        for r in top:
            f.write(
                ...
                f"APT32={r.get('apt32_legacy_primary')} "
                f"layers={r.get('layer_count_daily')} "
                f"reason={r.get('v4_reason')}\n"
            )
...
def main():
    ...
    for device, root in ROOTS:
        ...
        if not root.exists():
            all_errors.append({"device": device, "path": str(root), "member": "", "error": "root_not_found"})
    ...
    write_csv(OUT / "90_excluded_files.csv", all_excluded, ["device", "path", "reason"])
    write_csv(OUT / "91_errors.csv", all_errors, ["device", "path", "member", "error"])
